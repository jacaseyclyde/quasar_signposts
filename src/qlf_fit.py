...
def _log_normalization(theta, completeness_fn, cosmo=cosmo, compl=1,
                       crts_based=False):
    # assume poisson errors on the number. if it's based on CRTS, match CRTS
    if crts_based:
        n_crts = np.sum(crts_pop['weight'])
        n_crts_i = np.random.poisson(n_crts)
        n_bh = np.sum(pop['weight']) * n_crts_i / n_crts
    else:
        n_bh = np.sum(pop['weight'])
    ...
    if callable(completeness_fn):
        log_l_min = completeness_fn(pop['z'])
...
def log_probability(theta, bounds, completeness_fn, cosmo=cosmo, compl=1,
    ...
    d0 = _log_normalization(theta, completeness_fn, cosmo=cosmo, compl=compl,
...
def log_likelihood(theta, completeness_fn):
    mdl = _fit_model(pop['log_l_bol'], pop['z'], *theta)
    ...
    res = np.nansum(pop['weight'] * (ln_num - ln_denom))
...
def initialize_params(init_bounds, param_bounds, nwalkers=32,
    ...
    d0 = _log_normalization(pos.T, completeness_fn=completeness_fn,
    ...
    while np.sum(valid) < nwalkers:  #
        ...
        d0_tmp = _log_normalization(pos_tmp.T, completeness_fn,
...
def mcmc_sampler(log_prob, bounds, completeness_fn=LOG_L_MIN, cosmo=cosmo,
                 compl=1, init_bounds=None, nwalkers=32, ncpus=mp.cpu_count(),
                 max_nsamples=10000, len_acorr=50, filename=None,
                 track_acorr=True, cont=True, crts_based=False
                 ):
    args = (bounds, completeness_fn, cosmo, compl, crts_based)
...
if __name__ == '__main__':
    ...
    _plot_qlf(post, hatch=HS[1], linestyle=LS[1], label="Fit")
